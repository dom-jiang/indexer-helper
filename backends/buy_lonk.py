...
import globals_buy_lonk
from buylonk_config import GlobalConfig
...
def handle_buy_buck_one(network_id, random_num):
    try:
        ...
        ret = conn.view_call(global_config.buylonk_contract, "get_available_fund_amount", b'')
        ...
        if amount_in > 0:
            ...
            if amount_in > 0:
                handle_buy_buck_two(network_id, 300)
            else:
...
def handle_flow(network_id, amount_in, random_num):
    ...
    buylonk_pool_one = {}
    for pool in pools:
        if pool["id"] == global_config.buylonk_pool_one:
            buylonk_pool_one = pool

    if buylonk_pool_one != {}:
        one_account_ids = buylonk_pool_one["token_account_ids"]
        one_amounts = buylonk_pool_one["amounts"]
        if one_account_ids[0] == global_config.buylonk_token_in_contract:
        ...
        one_amount_out = get_token_flow_ratio(amount_in, one_in_balance, one_out_balance, buylonk_pool_one["total_fee"])
        print("one_amount_out:", one_amount_out)
        one_min_amount_out = int(decimal_mult(one_amount_out, 0.997))
        action_one = {
            "pool_id": int(buylonk_pool_one["id"]),
    ...
    signer = globals_buy_lonk.get_signer_account(global_config.signer_account_id)
    burrow_handler = RpcHandler(signer, global_config.buylonk_contract)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        ...
        if network_id in ["MAINNET", "TESTNET", "DEVNET"]:
            ...
            handle_buy_buck_one(network_id, 300)
